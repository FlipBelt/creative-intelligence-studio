...
export function updateInput(p: Project, patch: Partial<Pick<Project,"brief"|"audience"|"output"|"product"|"bound"|"confirmed"|"refs"|"simple">>, now: string): Project {
...
export function directionGate(p: Project): string | null {
 ...
 if (!p.simple && !p.bound) return "先在产品理解中保存当前项目输入";
 if (!p.confirmed || !p.brief.trim()) return "先保存并确认创意需求";
 if (!p.simple && !p.refs.length) return "至少选择一项摄影参考";
 return null;
}
